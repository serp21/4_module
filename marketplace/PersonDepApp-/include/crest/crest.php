...
if(!empty($_REQUEST['DOMAIN'])){
	$R_DOMAIN = $_REQUEST['DOMAIN'];
} else if(!empty($_REQUEST['auth']['domain'])) {
	$R_DOMAIN = $_REQUEST['auth']['domain'];
} else {
	echo json_encode('Пустой запрос');
	exit;
}
...
class CRest
{
		...
		elseif($_REQUEST['PLACEMENT'] == 'DEFAULT')
		{
			...
			$result['install'] = static::setAppSettings(
				[
					...
					'domain' => htmlspecialchars($R_DOMAIN),
					'client_endpoint' => 'https://' . htmlspecialchars($R_DOMAIN) . '/rest/',
				],
				true
			);
		}
	...
	protected static function getSettingData()
	{
		...
		$return = [];
		if(file_exists(__DIR__ . '/'.htmlspecialchars($R_DOMAIN).'.json'))
		{
			$return = static::expandData(file_get_contents(__DIR__ . '/'.htmlspecialchars($R_DOMAIN).'.json'));
	...
	protected static function setSettingData($arSettings)
	{
		...
		return  (boolean)file_put_contents(__DIR__ . '/'.htmlspecialchars($R_DOMAIN).'.json', static::wrapData($arSettings));
	}
	...
	protected static function getSettingData()
	{
		...
		if(is_array($return))
		{
			...
			else
			{
				...
				$return['domain'] = htmlspecialchars($R_DOMAIN);
