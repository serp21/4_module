<?php
	...
	class CRest
	{
		...
		public static function installApp()
		{
            ...
            {
                sleep(1);
                $settings = static::call('app.option.get');
                
                #   Создание списков
                if(!isset($settings['result']['payList']) || !isset($settings['result']['salList']))
                {
                    $query      = array();
                    $list_id    = 0;
                    $zp_list_id = 0;
                    
                    $payList = static::call('lists.add', array('IBLOCK_TYPE_ID' => 'lists',
                                             'IBLOCK_CODE' => 'salary',
                                             'FIELDS' => array(
                    ...
                    if(!empty($payList['result']))
                    {
                        $zp_list_id = $payList['result'];
                        $query[] = array('method' => 'app.option.set', 'params' => array('salList' => $zp_list_id));
                        
                        #   Дата начисления
                        $query[] = array('method' => 'lists.field.add', 
                                             'params' => array('IBLOCK_TYPE_ID' => 'lists',
                                                               'IBLOCK_ID' => $zp_list_id,
                                                               'FIELDS' => array(
                        ...
                        #   Баланс на момент начисления
                        $query[] = array('method' => 'lists.field.add', 
                                             'params' => array('IBLOCK_TYPE_ID' => 'lists',
                                                               'IBLOCK_ID' => $zp_list_id,
                                                               'FIELDS' => array(
                                                                    'NAME' => 'Баланс на момент начисления',
                                                                    'TYPE' => 'N',
                                                                    'CODE' => 'salBalance',
                                                                    'SETTINGS' => array(
                                                                        'SHOW_ADD_FORM' => 'Y',
                    ...
                    sleep(1);
                    $list = static::call('lists.add', array('IBLOCK_TYPE_ID' => 'lists',
                                         'IBLOCK_CODE' => 'payments',
                                         'FIELDS' => array(
                                             'NAME' => 'Платежи',
                                             'DESCRIPTION' => 'Наличные/безналичные платежи',
                                             'SORT' => '10',
                                             'BIZPROC' => 'Y'
                                         )));
                    if(!empty($list['result']))
                    {
                        ...
                        $query[] = array('method' => 'app.option.set', 'params' => array('payList' =>  $list_id));
                        ...
                        $query[] = array('method' => 'lists.field.add', 
                                             'params' => array('IBLOCK_TYPE_ID' => 'lists',
                                                               ...
                                                               'FIELDS' => array(
                                                                    ...
                                                                    'USER_TYPE_SETTINGS' => array('COMPANY' => 'Y'),
                                                                    'SORT' => 70
                                                               )));
                        #   Расчётный счёт
                        $query[] = array('method' => 'lists.field.add', 
                        ...
                        $query[] = array('method' => 'lists.field.add', 
                                             'params' => array('IBLOCK_TYPE_ID' => 'lists',
                                                               ...
                                                               'FIELDS' => array(
                                                                    ...
                                                                    'USER_TYPE_SETTINGS' => array('LEAD' => 'Y', 'DEAL' => 'Y', 'VISIBLE' => 'Y'),
                                                                    'SORT' => 100
                                                               )));
                        #   ID счёта
                        $query[] = array('method' => 'lists.field.add', 
                    ...
                    sleep(1);
                    $transList = static::call('lists.add', array('IBLOCK_TYPE_ID' => 'lists',
                                              'IBLOCK_CODE' => 'transfer',
                                              'FIELDS' => array(
                                                  'NAME' => 'Переводы',
                                                  'DESCRIPTION' => 'Переводы денежных средств между сотрудниками',
                                                  'SORT' => '10',
                                                  'BIZPROC' => 'Y'
                                            )));
                    if(!empty($transList['result']))
                    {
                        $trans_list_id = $transList['result'];
                        $query[] = array('method' => 'app.option.set', 'params' => array('transList' => $trans_list_id));

                        $query[] = array('method' => 'lists.field.add',
                            'params' => array('IBLOCK_TYPE_ID' => 'lists',
                                'IBLOCK_ID' => $trans_list_id,
                                'FIELDS' => array(
                                    'NAME' => 'Дата перевода',
                                    'TYPE' => 'S:Date',
                                    'CODE' => 'transDate',
                    ...
                    {
                        sleep(1);
                        static::callBatch($query);

                        sleep(1);
                        $setCode = array();
                        $r = CRest::call('lists.field.get', array('IBLOCK_TYPE_ID' => 'lists', 'IBLOCK_ID' => array($list_id, $zp_list_id)));
                        if(!empty($r['result']))
                        {
                            foreach($r['result'] as $prop => $val)
                            {
                                $setCode[$val['IBLOCK_ID']][$val['CODE']] = $prop;
                            }
                            sleep(1);
                            CRest::call('app.option.set', array('listCode' => serialize($setCode)));
                        }
                    }
                }
            }
