            ...
            if(isset($_POST['getRespPay']) && !empty($_POST['getRespPay']))
            {
                ...
                if($sql->num_rows > 0)
                {
                    ...
                    if(!empty($tmpAcc))
                    {
                        ...
                        if(!empty($arOut))
                        {
                            $sql = $this->db->query("SELECT `ID`, CONCAT(`LAST_NAME`, ' ', `NAME`) AS `user` FROM `b_user` WHERE `ID` IN(".implode(',', $arOut).")");
                            if($sql->num_rows > 0)
                            {
                                while($result = $sql->fetch_assoc())
                                {
...
*/
            ...
            if(isset($_POST['getInfo']))
            {
                ...
                else
                {
                    ...
                    if($sql->num_rows > 0)
                    {
                        while($result = $sql->fetch_assoc())
                        {
                            if(trim($result['ls']) == '40802810729170004115' && $_SESSION['bitAppPayment']['ADMIN'] != 1)
                                continue;
    ...
    public function getSafe()
    {
        if($_SESSION['bitAppPayment']['ID'] == 26 || $_SESSION['bitAppPayment']['ID'] == 3 || $_SESSION['bitAppPayment']['ID'] == 7 || $_SESSION['bitAppPayment']['ID'] == 121 || $_SESSION['bitAppPayment']['ID'] == 128)
        {
            ...
            $sql = $this->db->query("SELECT `e`.`ID`, `p1`.`VALUE_NUM` AS `sum`, CONCAT(`u`.`LAST_NAME`, ' ', LEFT(`u`.`NAME`, 1), '.') AS `user`, `p2`.`VALUE` AS `user_id`,
                                     ...
                                     LEFT JOIN `b_user` AS `u` ON `u`.`ID` = `p2`.`VALUE`
                                     WHERE `e`.`IBLOCK_ID` = 28 AND `p5`.`VALUE` = 1 AND `p3`.`VALUE` BETWEEN '".date('Y-m-d', strtotime('-7 day'))." 00:00:00' AND '".date('Y-m-d')." 23:59:59'
                                     ORDER BY `e`.`ID` DESC");
            if($sql->num_rows > 0)
            {
                while($result = $sql->fetch_assoc())
    ...
    public function getSafeHis()
    {
        ...
        if($sql->num_rows > 0)
        {
            while($result = $sql->fetch_assoc())
            {
                $out .= '<tr><td>'.$result['ID'].'</td><td>'.date('d.m.Y', strtotime($result['date'])).'</td><td>'.number_format($result['sum'], 2, '.', ' ').'</td><td>'.htmlspecialchars($result['comment']).'</td><td>'.htmlspecialchars($result['user']).'</td></tr>';
            }
        }
                ...
                foreach($tmpP as $id_p)
                {
                    ...
                    if(!empty($payment))
                    {
                        ...
                        if($post['type'] == 283)
                        {
                            ...
                            $insert['PP']             = $payment['pp'];#PP
                            $insert['DATE']           = $payment['date'];#DATE
                            $insert['SUM']            = $payment['sum'];#SUM
                            $insert['COMP']           = $payment['company'];#COMP
                            $insert['INN']            = $payment['INN'];#INN
                            $insert['CONTR']          = trim($payment['contr_name']);#CONTR
                            $insert['NAZN']           = trim($payment['naznach']);#NAZN
                            $insert['LS']             = $payment['LS'];#LS
                            $insert['DEAL']           = $deal;#DEAL
                            $insert['INVOICE']        = $payment['inv_id'];#INVOICE
                            $insert['OPERATOR']       = $_SESSION['bitAppPayment']['ID'];#OPERATOR
                            $insert['DATA_OBRABOTKI'] = date('Y-m-d');#DATA_OBRABOTKI
                            $insert['TASK']           = (int)$post['task'];#TASK
                            $insert['TASK_LINK']      = '<a href="/company/personal/user/'.$_SESSION['bitAppPayment']['ID'].'/tasks/task/view/'.(int)$post['task'].'/">'.htmlspecialchars($task).'</a>';#TASK_LINK
                            $insert['INVOICE_LINK']   = $payment['inv_link'];#INVOICE_LINK
                            $insert['SUM_OSN']        = $payment['sum_osn'];#SUM_OSN
                            $insert['ZP']             = $payment['ZP'];#ZP
                            $insert['comment']        = trim($payment['comment']);#ZP
                            $insert['act']            = 'update';
                            $insert['status']         = 'wait';
                            */
                        }
                        ...
                        if(!empty($insert))
                        {
                            file_put_contents('./quenue.db', "\r\n".implode('|', $insert), FILE_APPEND);
                            $ins = 1;
                        }
                        */
                    }
