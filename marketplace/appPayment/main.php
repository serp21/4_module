...
Class M_BitAppPayment
{
    ...
    public $safe     = array();
    public $safeHis  = '';
    public $dateUpload = '2024-03-01';

    public function __construct()
    {
        require_once _PATH .'/crest/crest.php';
        $file = json_decode(file_get_contents(_PATH .'/crest/settings.json'));
        if($file->expires > 0 && $file->expires <= time())
        {
            $ch = curl_init();
        ...
        $this->db = new mysqli('localhost', 'appbitrix', '<bnhbrc21:', 'sitemanager');
        ...
        if(isset($_REQUEST['AUTH_ID']))
        {
            ...
            if(isset($result->result->ID))
            {
                ...
                $_SESSION['bitAppPayment']['ADMIN']     = ((int)$result->result->ID == USER_NIKITIN || (int)$result->result->ID == USER_NIKITIN) ? 1 : (int)$result->result->ADMIN;
                ...
                if($_SESSION['bitAppPayment']['ID'] == USER_SLUNIAEV)
...
if($_SESSION['bitAppPayment']['ID'] == USER_MITIN)
    $_SESSION['bitAppPayment']['ADMIN'] = 1;
        ...
        elseif(isset($_POST['saveSumAcc']))
            ...
            if($_SESSION['bitAppPayment']['ADMIN'] != 1 && file_exists(_PATH .'/access/'.$_SESSION['bitAppPayment']['ID']))
            {
                ...
                $sql = $this->db->query("SELECT * FROM `b_user_group` WHERE `GROUP_ID` = ".GROUPS_PAYS." AND `USER_ID` = ".(int)$_SESSION['bitAppPayment']['ID']);
                if($sql->num_rows > 0)
                ...
                #{
                    ...
                    #{
                        $sql = $this->db->query("SELECT DISTINCT(`IBLOCK_ELEMENT_ID`) FROM `b_iblock_element_property` 
                            WHERE `IBLOCK_PROPERTY_ID` = ".IB_ZARABOTNAYA_PLATA_SOTRUDNIK." AND `VALUE` = ".(int)$_SESSION['bitAppPayment']['ID']);
                        if($sql->num_rows > 0)
                        {
        ...
        if($_SESSION['bitAppPayment']['ADMIN'] == 1)
        {
            ...
            $sql = $this->db->query("SELECT `u`.`ID`, `u`.`LAST_NAME`, `u`.`NAME`, `d`.`VALUE_INT` AS `dept`
                                     ...
                                     WHERE `u`.`ACTIVE` = 'Y' 
                                     AND (`u`.`NAME` IS NOT NULL OR `u`.`NAME` != '') AND (`u`.`LAST_NAME` IS NOT NULL OR `u`.`LAST_NAME` != '')
...
*/
        ...
        if($_SESSION['bitAppPayment']['ID'] == USER_BELOGLAZOV || $_SESSION['bitAppPayment']['ID'] == USER_ROVNOV_A 
        || $_SESSION['bitAppPayment']['ID'] == USER_ROVNOV_E || $_SESSION['bitAppPayment']['ID'] == USER_NIKITIN || $_SESSION['bitAppPayment']['ID'] == USER_NIKITINA)
            $this->getSafe();
            ...
            if(($_SESSION['bitAppPayment']['ADMIN'] == 1 || $_SESSION['bitAppPayment']['ACCESS']['nal'] == 1) && ($_POST['type'] == 'task' || $_POST['type'] == 'zp' || $_POST['type'] == 'user' || $_POST['type'] == 'invoice' || $_POST['type'] == 'comand') && $_POST['sum'] != 0 && $_POST['task'] > 0)
            {
                ...
                if($_POST['sum'] < 0)
                {
                    $sql = $this->db->query("SELECT SUM(`p1`.`VALUE_NUM`) AS `sum`
                                             ...
                                             LEFT JOIN `b_iblock_element_property` AS `p2` ON `p2`.`IBLOCK_ELEMENT_ID` = `e`.`ID` AND `p2`.`IBLOCK_PROPERTY_ID` = ".IB_PLATEJI_OPERATOR."
                                             LEFT JOIN `b_iblock_element_property` AS `p3` ON `p3`.`IBLOCK_ELEMENT_ID` = `e`.`ID` AND `p3`.`IBLOCK_PROPERTY_ID` = ".IB_PLATEJI_NALICHNYY_PLATEJ."
                                             WHERE `e`.`IBLOCK_ID` = ".IB_PLATEJI." AND `p2`.`VALUE` = ".(int)$_SESSION['bitAppPayment']['ID']." AND `p3`.`VALUE` = '1'");
                ...
                if(!empty($query))
                {
                    CRest::call('lists.element.add', $query);
                    $out['status'] = 1;
...
*/
            ...
            if(isset($_POST['getPay']))
            {
                if($_POST['id'] > 0)
                {
                    ...
                    {
                        $sql = $this->db->query("SELECT `e`.`ID`, `e0`.`VALUE` AS `pp`, `e1`.`VALUE` AS `date`, `e2`.`VALUE_NUM` AS `sum`, `e12`.`VALUE` AS `id_rent`,
                                                        ...
                                                        `mtr`.`ID` AS `rent2`, `mtr`.`TITLE` AS `rent_name2`, CONCAT(`u2`.`LAST_NAME`, ' ', `u2`.`NAME`) AS `rent_user`
                                                 FROM `b_iblock_element` AS `e`
                                                 ...
                                                 LEFT JOIN `b_crm_dynamic_items_176` AS `r` ON `r`.`ID` = REPLACE(`e12`.`VALUE`, 'Tb0_', '')
												 LEFT JOIN `b_crm_dynamic_items_133` AS `mtr` ON `mtr`.`ID` = REPLACE(`e12`.`VALUE`, 'T85_', '')
                        ...
                        if($sql->num_rows > 0)
                        {
                            ...
                            $sql2 = $this->db->query("SELECT `e`.`ID`, `e0`.`VALUE` AS `pp`, `e1`.`VALUE` AS `date`, `e2`.`VALUE_NUM` AS `sum`, 
                                                             ...
                                                             `e8`.`VALUE` AS `ls`, `i`.`ORDER_TOPIC`, LEFT(`t`.`TITLE`, 60) AS `task`, `e12`.`VALUE` AS `id_rent`,
                                                             CONCAT(`u`.`LAST_NAME`, ' ', `u`.`NAME`) AS `user`, `e10`.`VALUE` AS `id_zp`,`r`.`ID` AS `rent`, `r`.`TITLE` AS `rent_name`,
                                                             `mtr`.`ID` AS `rent2`, `mtr`.`TITLE` AS `rent_name2`, CONCAT(`u2`.`LAST_NAME`, ' ', `u2`.`NAME`) AS `rent_user`
                                                      FROM `b_iblock_element` AS `e`
                                                      LEFT JOIN `b_iblock_element_property` AS `e0` ON `e0`.`IBLOCK_ELEMENT_ID` = `e`.`ID` AND `e0`.`IBLOCK_PROPERTY_ID` = 149
                            ...
                            if($sql2->num_rows > 0)
                            {
                                while($result2 = $sql2->fetch_assoc())
                                {
                                    if($_SESSION['bitAppPayment']['ADMIN'] == 1)
                                    {
                                        ...
                                        if(!empty($result2['id_rent']))
                                        {
                                            if(strpos($result2['id_rent'], 'T85') === false)
                                                $rent = ' / Аренда ТС: '.$result2['rent_name'];
                                            else
                                                $rent = ' / Аренда ТС: '.$result2['rent_name2'];
                                        }
                                        else
                                            $rent = '';
                                        ...
                                        $color  = (empty($result2['ID'])) ? 'style="color:red"' : '';
                            ...
                            else
                            {
                                if($_SESSION['bitAppPayment']['ADMIN'] == 1)
                                {
                                    ...
                                    $active = (empty($task) && empty($inv) && empty($zp) && empty($rent)) ? 'selected="selected"' : '';
                                    $color = (empty($result['ID'])) ? 'style="color:red"' : '';
                    ...
                    if($_POST['type'] == 274)
                    {
                        if($_SESSION['bitAppPayment']['ADMIN'] == 1 || $_SESSION['bitAppPayment']['ACCESS']['nal'] == 1)
                        {
                            $sql = $this->db->query("SELECT `e`.`ID`, `e`.`NAME`, `e1`.`VALUE` AS `date`, `e3`.`VALUE` AS `deal_id`, `d`.`TITLE` AS `deal`, 
                                                            ...
                                                            CONCAT(`u3`.`LAST_NAME`, ' ', LEFT(`u3`.`NAME`, 1),'.', LEFT(`u3`.`SECOND_NAME`, 1),'.') AS `salary`, 
                                                            `r`.`ID` AS `rent`, `r`.`TITLE` AS `rent_name`, `mtr`.`ID` AS `rent2`, `mtr`.`TITLE` AS `rent_name2`, CONCAT(`u4`.`LAST_NAME`, ' ', `u4`.`NAME`) AS `rent_user`
                                                     FROM `b_iblock_element` AS `e`
                                                     LEFT JOIN `b_iblock_element_property` AS `e1` ON `e1`.`IBLOCK_ELEMENT_ID` = `e`.`ID` AND `e1`.`IBLOCK_PROPERTY_ID` = 146
            ...
            if(isset($_POST['loadInfo']))
            {
                ...
                #   RESPONSE
                $sql = $this->db->query("SELECT `u`.`ID`, CONCAT(`u`.`LAST_NAME`, ' ', `u`.`NAME`, ' ', `u`.`SECOND_NAME`) AS `NAME`
                                         FROM `b_user` AS `u`
                                         LEFT JOIN `b_utm_user` AS `um` ON `um`.`VALUE_ID` = `u`.`ID`
                                         WHERE `u`.`ACTIVE` = 'Y' AND `u`.`LAST_LOGIN` IS NOT NULL AND `um`.`VALUE_INT` > 0
                                         GROUP BY `u`.`ID`
                ...
                if($sql->num_rows > 0)
                {
                    while($result = $sql->fetch_assoc())
                    {
                        $return['resp'] .= '<div class="respList" style="margin:1px;cursor:pointer;" onclick="$(\'#idResp\').val('.$result['ID'].');$(\'#f_resp\').val(\''.htmlspecialchars($result['NAME']).'\');$(\'#respList\').hide();">'.htmlspecialchars($result['NAME']).'</div>';
                    }
                }
            ...
            if(isset($_POST['getInfo']))
            {
                ...
                {
                    $sql = $this->db->query("SELECT `e`.`ID`, `e`.`NAME`, `e1`.`VALUE` AS `date`, `d`.`ID` AS `deal_id`, `d`.`TITLE` AS `deal`, `mtr`.`ID` AS `mtr_id`, `mtr`.`TITLE` AS `mtr`, 
                                                    `cmp`.`ID` AS `cmp_id`, `cmp`.`TITLE` AS `cmpTitle`, `e14`.`VALUE` AS `id_rent`,
                                                    ...
                                                    `e13`.`VALUE` AS `nazn`, `r`.`ID` AS `rent`, `r`.`TITLE` AS `rent_name`, `mtr2`.`ID` AS `rent2`, `mtr2`.`TITLE` AS `rent_name2`,
                                             ...
                                             LEFT JOIN `b_crm_dynamic_items_176` AS `r` ON `r`.`ID` = REPLACE(`e14`.`VALUE`, 'Tb0_', '')
											 LEFT JOIN `b_crm_dynamic_items_133` AS `mtr2` ON `mtr2`.`ID` = REPLACE(`e14`.`VALUE`, 'T85_', '')
                                             LEFT JOIN `b_user`        AS `u2` ON `u2`.`ID` = `r`.`ASSIGNED_BY_ID`
                                             WHERE ".$where."
                                             GROUP BY `e`.`ID`
                                             ORDER BY ".$order);
                    if($sql->num_rows > 0)
                    {
                        ...
                        if(!empty($logID) && $_SESSION['bitAppPayment']['ADMIN'] == 1)
                        {
                            $sqlHis = $this->db->query("SELECT `e`.`ID`, `p1`.`VALUE` AS `date`, `p2`.`VALUE` AS `id_pay`, `p3`.`VALUE` AS `CRM`, 
                                                               ...
                                                               `r`.`ID` AS `rent`, `r`.`TITLE` AS `rent_name`, `mtr`.`ID` AS `rent2`, `mtr`.`TITLE` AS `rent_name2`,
                                                               CONCAT(`u2`.`LAST_NAME`, ' ', `u2`.`NAME`) AS `rent_user`, `p11`.`VALUE` AS `id_rent`
                                                        FROM `b_iblock_element` AS `e`
                                                        ...
                                                        LEFT JOIN `b_crm_dynamic_items_176` AS `r` ON `r`.`ID` = REPLACE(`p11`.`VALUE`, 'Tb0_', '')
											            LEFT JOIN `b_crm_dynamic_items_133` AS `mtr` ON `mtr`.`ID` = REPLACE(`p11`.`VALUE`, 'T85_', '')
                                                        LEFT JOIN `b_user`        AS `u2` ON `u2`.`ID` = `r`.`ASSIGNED_BY_ID`
                        ...
                        foreach($arNal as $result)
                        {
                            ...
                            $errStr   = ((int)$result['task_id'] == 0 && (int)$result['invoice'] == 0 && (int)$result['zp_id'] == 0 && empty($result['id_rent'])) ? 'class="alert-danger"' : '';
                ...
                else
                {
                    ...
                    $sql = $this->db->query("SELECT `e`.`ID`, `e1`.`VALUE` AS `date`, `e0`.`VALUE` AS `pp`, `e2`.`VALUE_NUM` AS `sum`, `e13`.`VALUE_NUM` AS `sum_osn`, `e3`.`VALUE` AS `contragent`, `c`.`TITLE` AS `company`,
                                                    ...
                                                    CONCAT(`u`.`LAST_NAME`, ' ', `u`.`NAME`) AS `card`, `e20`.`VALUE` AS `id_rent`,
                                                    REPLACE(`e5`.`VALUE`, 'T85_', '') AS `mtr_id`, `r`.`ID` AS `rent`, `r`.`TITLE` AS `rent_name`, `mtr`.`ID` AS `rent2`, `mtr`.`TITLE` AS `rent_name2`,
                                             ...
                                             LEFT JOIN `b_crm_dynamic_items_176` AS `r` ON `r`.`ID` = REPLACE(`e20`.`VALUE`, 'Tb0_', '')
											 LEFT JOIN `b_crm_dynamic_items_133` AS `mtr` ON `mtr`.`ID` = REPLACE(`e20`.`VALUE`, 'T85_', '')
                                             LEFT JOIN `b_user`        AS `u2` ON `u2`.`ID` = `r`.`ASSIGNED_BY_ID`
                                             WHERE ".$where."
                                             GROUP BY `e`.`ID`
                                        ...
                                        foreach($val2 as $ls => $val3)
                                        {
                                            foreach($val3 as $contr => $val4)
                                            {
                                                ...
                                                if($cnt > 1)
                                                {
                                                    ...
                                                    $errStr   = ((int)$first['task_id'] == 0 && (int)$first['invoice'] == 0 && (int)$first['id_zp'] == 0 && empty($first['id_rent'])) ? 'class="alert-danger"' : '';
                                                    ...
                                                    foreach($val4['IDS'] as $id_s => $value_s)
                                                    {
                                                        ...
                                                        $errStr   = ((int)$value_s['task_id'] == 0 && (int)$value_s['invoice'] == 0 && (int)$value_s['id_zp'] == 0 && empty($value_s['id_rent'])) ? 'class="alert-danger"' : '';
                                                        ...
                                                        if(!empty($value_s['id_rent']))
                                                        {
                                                            if(strpos($value_s['id_rent'], 'T85') === false)
                                                                $crm = 'Аренда ТС: <a target="_blank" href="/crm/type/176/details/'.(int)$value_s['rent'].'/">'.htmlspecialchars($value_s['rent_name']).'</a>';
                                                            else
                                                                $crm = 'Аренда ТС: <a target="_blank" href="/crm/type/133/details/'.(int)$value_s['rent2'].'/">'.htmlspecialchars($value_s['rent_name2']).'</a>';
                                                        }
    ...
    public function getRentList()
    {
        ...
        if($sql->num_rows > 0)
        {
            while($result = $sql->fetch_assoc())
            {
                if($result['auto'] > 0)
                {
                    $arRent['Tb0_'.$result['auto']]['name']  = $result['auto_name'];
                    $arRent['Tb0_'.$result['auto']]['assig'] = $result['ASSIGNED_BY_ID'];
                    $arRent['Tb0_'.$result['auto']]['assig_name'] = $result['user'];
                    $arRent['Tb0_'.$result['auto']]['sum']  = $result['sum'];
                }
            }
        }

        $sql = $this->db->query("SELECT `a`.`ID` AS `auto`, `p2`.`VALUE`, `a`.`TITLE` AS `auto_name`, ROUND(SUM(`p1`.`VALUE`), 2) AS `sum`, `a`.`ASSIGNED_BY_ID`, CONCAT(`u`.`LAST_NAME`, ' ', `u`.`NAME`) AS `user` 
                                 FROM `b_iblock_element_property` AS `p1`
                                 LEFT JOIN `b_iblock_element_property` AS `p2` ON `p2`.`IBLOCK_ELEMENT_ID` = `p1`.`IBLOCK_ELEMENT_ID` AND `p2`.`IBLOCK_PROPERTY_ID` = 490
                                 LEFT JOIN `b_crm_dynamic_items_133` AS `a` ON `a`.`ID` = `p2`.`VALUE`
                                 LEFT JOIN `b_user` AS `u` ON `u`.`ID` = `a`.`ASSIGNED_BY_ID`
                                 WHERE `p1`.`IBLOCK_PROPERTY_ID` = 432
                                 GROUP BY `p2`.`VALUE`");
        if($sql->num_rows > 0)
        {
            while($result = $sql->fetch_assoc())
            {
                if($result['auto'] > 0)
                {
                    $arRentMtr['T85_'.$result['auto']]['name']  = $result['auto_name'];
                    $arRentMtr['T85_'.$result['auto']]['assig'] = $result['ASSIGNED_BY_ID'];
                    $arRentMtr['T85_'.$result['auto']]['assig_name'] = $result['user'];
                    $arRentMtr['T85_'.$result['auto']]['sum']  = $result['sum'];
                }
            }
        }
        ...
        {
            $keysRent = array();
            foreach($arRent as $idR => $keyR)
            {
                $keysRent[] = "'".$idR."'";
                $keysRent[] = str_replace('Tb0_', '', $idR);
            }
            ...
            $sql = $this->db->query("SELECT `p2`.`VALUE` AS `rent`, SUM(`p1`.`VALUE`) AS `sum`
                                     ...
                                     WHERE `e`.`IBLOCK_ID` = 28 AND `p2`.`VALUE` IN(".implode(',', $keysRent).")
                                     GROUP BY `p2`.`VALUE`");
            if($sql->num_rows > 0)
            {
                while($result = $sql->fetch_assoc())
        ...
        if(!empty($arRentMtr))
        {
            ...
            if($sql->num_rows > 0)
            {
                while($result = $sql->fetch_assoc())
                {
                    $arRentMtr[$result['rent']]['sum'] += $result['sum'];
                }
            }
        }

        $rent = array_merge($arRent, $arRentMtr);
        if(!empty($rent))
        {
            foreach($rent as $id => $r)
            {
                if($r['sum'] != 0)
    ...
    public function getAcc()
    {
        ...
        $sql = $this->db->query("SELECT SUM(`e1`.`VALUE_NUM`) AS `SUM`, `e2`.`VALUE` AS `ACCOUNT`
                                FROM `b_iblock_element` AS `e`
                                ...
                                WHERE `e`.`IBLOCK_ID` = 28 AND `e2`.`VALUE` > 0
                                GROUP BY `e2`.`VALUE`");
        if($sql->num_rows > 0)
        {
            while($result = $sql->fetch_assoc())
        ...
        if($sql->num_rows > 0)
        {
            while($result = $sql->fetch_assoc())
            {
                if($_SESSION['bitAppPayment']['ADMIN'] == 1 || in_array($result['ID'], $_SESSION['bitAppPayment']['ACCESS']['acc']))
                {
                    ...
                    if(!isset($_POST['getAccessAcc']) && $_SESSION['bitAppPayment']['sumAcc'] == 0 && $company[$result['ENTITY_ID']]['ACCOUNT'][$result['ID']]['BALANCE'] <= 0)
                    {
        ...
        if($_SESSION['bitAppPayment']['ADMIN'] == 1 || $_SESSION['bitAppPayment']['ACCESS']['vb'] == 1)
        {
            ...
            else
            {
                ...
                if(move_uploaded_file($_FILES['txt']['tmp_name'], './upload/'.$name))
                {
                    ...
                    preg_match('#1CClientBankExchange(.*)КонецРасчСчет#Us', $file, $out);
...
*/
    ...
    public function savePart($data)
    {
        ...
        if($data['PFpayId'] > 0 && $data['PFpartTask'] > 0)
        {
            ...
            $sql = $this->db->query("SELECT `VALUE` FROM `b_iblock_element_property` WHERE `IBLOCK_PROPERTY_ID` = 147 AND `IBLOCK_ELEMENT_ID` = ".(int)$data['PFpayId']);
            if($sql->num_rows > 0)
            {
                $result = $sql->fetch_assoc();
                $data['PFpartSum'] = ($result['VALUE'] < 0) ? abs($data['PFpartSum']) * -1 : abs($data['PFpartSum']);
            }
            ...
            if($data['selTypeList'] == 5 && $data['PFpartTask'] > 0 && ($_SESSION['bitAppPayment']['ADMIN'] == 1 || $_SESSION['bitAppPayment']['ACCESS']['part'] == 1))
            {
                ...
                #{
                    ...
                    $arPayFields['PROPERTY_436'] = $data['PFpartTask'];
    ...
    public function getZP($ids = array())
    {
        $return = array();
        $paid   = array();
            
            $where = (!empty($ids)) ? " AND `e0`.`VALUE` IN(".implode(',', $ids).")" : " AND `e0`.`VALUE` > 0";
            ...
            $where = (!empty($ids)) ? " AND `e0`.`VALUE_NUM` > 0 AND `e`.`ID` IN(".implode(',', $ids).")" : " AND `e0`.`VALUE_NUM` > 0";
            $sql = $this->db->query("SELECT `e`.`ID`, `e2`.`VALUE` AS `date`, `e0`.`VALUE_NUM` AS `ZP`, `e3`.`VALUE_NUM` AS `ZP2`, CONCAT(`u`.`LAST_NAME`, ' ', `u`.`NAME`, ' ', `u`.`SECOND_NAME`) AS `user`
                                     FROM `b_iblock_element` AS `e`
                                     LEFT JOIN `b_iblock_element_property` AS `e0` ON `e0`.`IBLOCK_ELEMENT_ID` = `e`.`ID` AND `e0`.`IBLOCK_PROPERTY_ID` = 138
                                     LEFT JOIN `b_iblock_element_property` AS `e3` ON `e3`.`IBLOCK_ELEMENT_ID` = `e`.`ID` AND `e3`.`IBLOCK_PROPERTY_ID` = 258
                                     LEFT JOIN `b_iblock_element_property` AS `e1` ON `e1`.`IBLOCK_ELEMENT_ID` = `e`.`ID` AND `e1`.`IBLOCK_PROPERTY_ID` = 142
            ...
            if($sql->num_rows > 0)
            {
                while($result = $sql->fetch_assoc())
                {
                    $return[$result['ID']]['date']  = $result['date'];
                    $return[$result['ID']]['user']  = $result['user'];
                    $return[$result['ID']]['paid']  = (isset($paid[$result['ID']])) ? $paid[$result['ID']] : 0;
                    $return[$result['ID']]['oklad'] = $result['ZP'];
                    $return[$result['ID']]['prem']  = $result['ZP2'];
                }
            }
    ...
    #   Список зарплаты
    public function zpList($type = 0, $arr = 0, $idP = 0)
    {
        $return  = ($type == 0 || $type == 2) ? '' : '<optgroup label="{DEBT}"><option></option></optgroup>';
        $debtSum = 0;
        $retSum  = 0;
        
        #   Все начисления с указанием остатка
        if($_SESSION['bitAppPayment']['ADMIN'] == 1 || isset($_SESSION['bitAppPayment']['ACCESS']['zp']) && $_SESSION['bitAppPayment']['ACCESS']['zp'] == 1)
        {
            ...
            if(!empty($data))
            {
                foreach($data as $id => $zpList)
                {
                    if($_SESSION['bitAppPayment']['ADMIN'] == 1)
                        $zp = $zpList['oklad'] + $zpList['prem'] + $zpList['paid'];
                    else
                        $zp = $zpList['oklad'] + $zpList['paid'];

                    if((int)$zp >= 500)
                    {
                        $debtSum += $zp;
                        $zp = 'Оклад: '.number_format($zpList['oklad'], 2, '.', ' ').' / Остаток: '.number_format($zp, 2, '.', ' ');

                        if($type == 0)
                            $return .= '<div class="objList" onclick="$(\'#PFpartTask\').val('.(int)$id.');$(\'.objList\').removeClass(\'alert-success\');$(this).addClass(\'alert-success\')">'.date('m/Y', strtotime($zpList['date'])).' '.htmlspecialchars($zpList['user'] ?? '').' ('.$zp.')</div>';
                        elseif($type == 2)
                            $return .= '<div class="objList" onclick="$(\'#PFpartTask2\').val('.(int)$id.');$(\'.objList\').removeClass(\'alert-success\');$(this).addClass(\'alert-success\')">'.date('m/Y', strtotime($zpList['date'])).' '.htmlspecialchars($zpList['user'] ?? '').' ('.$zp.')</div>';
                        else
                            $return .= '<option value="'.(int)$id.'">'.date('m/Y', strtotime($zpList['date'])).' '.htmlspecialchars($zpList['user'] ?? '').' ('.$zp.')</option>';
                    }
                }
            }
        }
        
        #   Все начисления без указания остатка
        if(isset($_SESSION['bitAppPayment']['ACCESS']['zp1']) && $_SESSION['bitAppPayment']['ACCESS']['zp1'] == 1)
        {
            $data = $this->getZP();
            ...
            if(!empty($data))
            {
                ...
                {                  
                    ...
                    {
                        if($type == 0)
                            $return .= '<div class="objList" onclick="$(\'#PFpartTask\').val('.(int)$id.');$(\'.objList\').removeClass(\'alert-success\');$(this).addClass(\'alert-success\')">'.date('m/Y', strtotime($zpList['date'])).' '.htmlspecialchars($zpList['user'] ?? '').'</div>';
                        elseif($type == 2)
                            $return .= '<div class="objList" onclick="$(\'#PFpartTask2\').val('.(int)$id.');$(\'.objList\').removeClass(\'alert-success\');$(this).addClass(\'alert-success\')">'.date('m/Y', strtotime($zpList['date'])).' '.htmlspecialchars($zpList['user'] ?? '').'</div>';
                        else
                            $return .= '<option value="'.(int)$id.'">'.date('m/Y', strtotime($zpList['date'])).' '.htmlspecialchars($zpList['user'] ?? '').'</option>';
                    }
                }
            }
        }
        ...
        {
            $data = $this->getZP($_SESSION['bitAppPayment']['arZPlist']);
            ...
            if(!empty($data))
            {
                foreach($data as $id => $zpList)
                {
                    ...
                    {
                        $debtSum += $zp;
                        $zp = 'Оклад: '.number_format($zpList['oklad'], 2, '.', ' ').' / Премия: '.number_format($zpList['prem'], 2, '.', ' ').' / Остаток: '.number_format($zp, 2, '.', ' ');
            ...
            if(!empty($arDep))
            {
                ...
                if($sql->num_rows > 0)
                {
                    ...
                    if($sql->num_rows > 0)
                    {
                        ...
                        $data = $this->getZP($arZP);
                        if(!empty($data))
                        {
                            foreach($data as $id => $zpList)
                            {
                                $zp = (abs($zpList['paid']) > $zpList['oklad']) ? $zpList['paid'] + $zpList['oklad'] + $zpList['prem'] : $zpList['prem'];
                                ...
                                if((int)$zp >= 500)
                                {
                                    ...
                                    $zp = 'Премия: '.number_format($zpList['prem'], 2, '.', ' ').' / Остаток: '.number_format($zp, 2, '.', ' ');
        ...
        if($post['f_part_r'] == 274)
        {
            if( $_SESSION['bitAppPayment']['ADMIN'] == 1 || $_SESSION['bitAppPayment']['ACCESS']['nal'] == 1)
            {
                ...
                else
                {
                    ...
                    {
                        if(isset($_POST['idResp']) && $_POST['idResp'] > 0)
                            $where[] = "`e7`.`VALUE` = ".(int)$_POST['idResp'];    
                        else
                            $where[] = "`e7`.`VALUE` IN(SELECT `ID` FROM `b_user` WHERE `NAME` LIKE '%".$this->db->real_escape_string($_POST['f_resp'])."%' OR `LAST_NAME` LIKE '%".$this->db->real_escape_string($_POST['f_resp'])."%' OR `SECOND_NAME` LIKE '%".$this->db->real_escape_string($_POST['f_resp'])."%')";
                    }
        ...
        else
        {
            if($_SESSION['bitAppPayment']['ADMIN'] == 1 || $_SESSION['bitAppPayment']['ACCESS']['bnal'] == 1)
            {
                ...
                else
                {
                    ...
                    {
                        if(isset($_POST['idResp']) && $_POST['idResp'] > 0)
                            $where[] = "`e17`.`VALUE` = ".(int)$_POST['idResp'];    
                        else
